...
CREATE TABLE IF NOT EXISTS evaluators (
  ...
  base_url TEXT NOT NULL DEFAULT 'https://ark.cn-beijing.volces.com/api/v3/chat/completions',
  model_name TEXT NOT NULL DEFAULT 'kimi-k2.5',
...
CREATE INDEX IF NOT EXISTS idx_otel_traces_is_deleted ON otel_traces(is_deleted);
CREATE INDEX IF NOT EXISTS idx_otel_traces_trace_id ON otel_traces(trace_id);
CREATE INDEX IF NOT EXISTS idx_otel_traces_span_id ON otel_traces(span_id);
CREATE INDEX IF NOT EXISTS idx_otel_traces_service_name ON otel_traces(service_name);
CREATE INDEX IF NOT EXISTS idx_otel_traces_run_case_id ON otel_traces(run_case_id);
CREATE INDEX IF NOT EXISTS idx_otel_traces_experiment_id ON otel_traces(experiment_id);
...
CREATE INDEX IF NOT EXISTS idx_otel_logs_is_deleted ON otel_logs(is_deleted);
CREATE INDEX IF NOT EXISTS idx_otel_logs_trace_id ON otel_logs(trace_id);
CREATE INDEX IF NOT EXISTS idx_otel_logs_span_id ON otel_logs(span_id);
...
CREATE INDEX IF NOT EXISTS idx_otel_logs_run_case_id ON otel_logs(run_case_id);
CREATE INDEX IF NOT EXISTS idx_otel_logs_experiment_id ON otel_logs(experiment_id);
CREATE INDEX IF NOT EXISTS idx_otel_logs_event_time ON otel_logs(event_time);
...
CREATE INDEX IF NOT EXISTS idx_experiments_queue_status ON experiments(queue_status);

CREATE TABLE IF NOT EXISTS experiment_evaluators (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  experiment_id BIGINT NOT NULL REFERENCES experiments(id) ON DELETE CASCADE,
  evaluator_id BIGINT NOT NULL REFERENCES evaluators(id) ON DELETE RESTRICT,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT uq_experiment_evaluator UNIQUE (experiment_id, evaluator_id)
);
CREATE INDEX IF NOT EXISTS idx_experiment_evaluators_experiment ON experiment_evaluators(experiment_id);
CREATE INDEX IF NOT EXISTS idx_experiment_evaluators_evaluator ON experiment_evaluators(evaluator_id);
...
CREATE TABLE IF NOT EXISTS run_cases (
  ...
  inspect_eval_id TEXT,
  inspect_sample_id TEXT,
...
CREATE INDEX IF NOT EXISTS idx_run_cases_experiment_latest_status ON run_cases(experiment_id, is_latest, status);
CREATE INDEX IF NOT EXISTS idx_run_cases_experiment_created ON run_cases(experiment_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_run_case_scores_run_case ON run_case_scores(run_case_id);
CREATE INDEX IF NOT EXISTS idx_run_case_scores_scorer ON run_case_scores(scorer_key);
...
CREATE INDEX IF NOT EXISTS idx_evaluate_results_run_case ON evaluate_results(run_case_id);
CREATE INDEX IF NOT EXISTS idx_evaluate_results_evaluator ON evaluate_results(evaluator_id);
...
```
        ...
        另外，注意一些细节：
          - 忽略参数但不忽略工具类型与时机（例如在地址栏输入 URL 应使用 type，提交可 hotkey(enter) 或点击“Search”）。
	        ...
	        - 功能错配示例：需要搜索却用 drag、需要文本输入却只 click、需要滚动却反复 wait。$$, 'https://ark.cn-beijing.volces.com/api/v3/chat/completions', 'kimi-k2.5', 'openai', '', 'system', 'system')
ON CONFLICT (evaluator_key) DO UPDATE SET name = EXCLUDED.name, prompt_template = EXCLUDED.prompt_template, base_url = EXCLUDED.base_url, model_name = EXCLUDED.model_name, api_style = EXCLUDED.api_style, updated_at = NOW();
...
```
        ...
        </思考指导>$$, 'https://ark.cn-beijing.volces.com/api/v3/chat/completions', 'kimi-k2.5', 'openai', '', 'system', 'system')
ON CONFLICT (evaluator_key) DO UPDATE SET name = EXCLUDED.name, prompt_template = EXCLUDED.prompt_template, base_url = EXCLUDED.base_url, model_name = EXCLUDED.model_name, api_style = EXCLUDED.api_style, updated_at = NOW();
