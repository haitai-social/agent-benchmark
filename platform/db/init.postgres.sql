...
CREATE TABLE IF NOT EXISTS evaluators (
  ...
  base_url TEXT NOT NULL DEFAULT 'https://ark.cn-beijing.volces.com/api/coding/v3',
...
ALTER TABLE evaluators ADD COLUMN IF NOT EXISTS api_style TEXT NOT NULL DEFAULT 'anthropic';
ALTER TABLE evaluators ADD COLUMN IF NOT EXISTS api_key TEXT NOT NULL DEFAULT '';
...
ALTER TABLE experiments ADD COLUMN IF NOT EXISTS queue_message_id TEXT;
ALTER TABLE experiments ADD COLUMN IF NOT EXISTS queued_at TIMESTAMPTZ;
ALTER TABLE experiments ADD COLUMN IF NOT EXISTS queue_status TEXT NOT NULL DEFAULT 'idle';
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
ALTER TABLE experiments DROP COLUMN IF EXISTS run_locked;
...
```
        ...
        另外，注意一些细节：
          - 忽略参数但不忽略工具类型与时机（例如在地址栏输入 URL 应使用 type，提交可 hotkey(enter) 或点击“Search”）。
	        ...
	        - 功能错配示例：需要搜索却用 drag、需要文本输入却只 click、需要滚动却反复 wait。$$, 'https://ark.cn-beijing.volces.com/api/coding/v3', 'kimi-k2.5', 'anthropic', '', 'system', 'system')
...
```
        ...
        </思考指导>$$, 'https://ark.cn-beijing.volces.com/api/coding/v3', 'kimi-k2.5', 'anthropic', '', 'system', 'system')
ON CONFLICT (evaluator_key) DO UPDATE SET name = EXCLUDED.name, prompt_template = EXCLUDED.prompt_template, base_url = EXCLUDED.base_url, model_name = EXCLUDED.model_name, api_style = EXCLUDED.api_style, updated_at = NOW();
